...
print("-ed -", [np.real( dens_state @ ed.Spin(1,jj,L) @ ed.Spin(1,jj+1,L) @ np.conj(dens_state) ) for jj in range(1,L)] ,",", np.real( dens_state @ ed.ising_ED(0,0,L) @ dens_state.conj() ) )
...
# var0 = np.tensordot(mpo_ham[0],mpo_ham[1],(2,0))
# var1 = np.tensordot(var0,mpo_ham[2],(4,0))
# var2 = np.tensordot(var1,mpo_ham[3],(6,0))
# print(var2.shape)
# print(np.transpose(var2,(0,2,4,6,1,3,5,7)).reshape(16,16))
# print(np.diag( ed.ising_ED(-1,0,L) ) )

# mps0 = np.transpose(mps_state.schmidt_matrix, mps_state.schmidt_matrix.conj(),(0,0))
# mps0 = np.transpose(mps_state.right_tensors[0], mps_state.right_tensors[0].conj(),(0,0)).transpose(0,2,1,3)

# var0 = np.tensordot(mps_state.right_tensors[0], mpo_ham[0],(1,1))
# var1 = np.tensordot(var0,mps_state.right_tensors[0].conj(),([0,2],[0,1]))
# print(var1.shape)
# print('- loop -', 0)
