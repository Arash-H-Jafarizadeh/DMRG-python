...
# X0 = np.tensordot( np.conj( mps_state.schmidt_matrix), mps_state.schmidt_matrix, (0,0)) 
# for el in range(L-1):
#     X0 = np.tensordot( X0, np.conj(mps_state.right_tensors[el]), (0,0) )
#     X0 = np.tensordot( X0, mps_state.right_tensors[el], (0,0) )
#     # print("----", X0.shape)
#     E0 = np.tensordot(X0, xop, ([0,2],[0,1]))
#     # print("----", E0.shape)
#     E0 = np.tensordot(E0, np.eye(2), ([0,1],[0,1]))
#     # print("----", E0.shape)
#     print("--", E0)
#     X0 = np.tensordot(X0, np.eye(2), ([0,2],[0,1]))
#     # print('\n')    
...
xxop = np.tensordot(xop,xop,0).transpose((0,2,1,3))
# print(xxop.shape,",",xxop.reshape((4,4)))

E0 = np.tensordot(mps_state.right_tensors[0], mps_state.right_tensors[1],(2,2))
print(" --0--", E0.shape)
E1 = np.tensordot(np.conj(mps_state.right_tensors[0]), np.conj(mps_state.right_tensors[1]),(2,2))
print(" --0--", E1.shape)
E3 = np.tensordot(E0, E1, ([0,3],[0,3]))
print(" --0--", E3.shape)
